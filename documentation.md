...
- RabbitMQ koristi publisher potvrde, durable red, dead-letter red, ograničen prefetch i inbox evidenciju obrađenih poruka. Worker ponavlja privremeno neuspjelu dostavu nakon 1, 2, 4 i 8 sekundi prije slanja u DLQ. Svaka poruka ima stabilan `Message-Id`, a worker prije svakog SMTP pokušaja preko MailHog API-ja provjerava da li je poruka već prihvaćena. Ako jeste, SMTP slanje se preskače i inbox zapis se završava; ako provjera nije dostupna, poruka ide kroz postojeći retry/DLQ tok umjesto da se naslijepo ponovo pošalje. MailHog koristi trajni `maildir` Docker volumen, pa evidencija prihvaćenih poruka ostaje dostupna i nakon restarta kontejnera. Ovaj mehanizam zatvara period mogućeg duplikata između SMTP prihvata i upisa obrađenog inbox zapisa u lokalnom Docker okruženju projekta.
